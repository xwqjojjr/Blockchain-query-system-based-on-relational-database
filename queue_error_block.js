...
connection.on('ready', function () {
	var AddressesSQL = 'select * from error_block';
                connection.query(AddressesSQL , function(err,result){
                if(err){
                 console.log('[INSERT ERROR] - ',err.message);
                 return;
                 }
	        for BLOCK_NUMBER in range result[i]
		        queue_blocks_from(result[i].block_number);
        });
});
